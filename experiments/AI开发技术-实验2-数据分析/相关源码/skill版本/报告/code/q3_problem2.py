...
ax.set_title(f"特征重要性 ({best_name})", fontweight="bold")
...
# Prompt版结果 (来自之前实验)
prompt_results = {
    "Gradient Boosting": (0.7671, 0.6841, 0.0352),
    "XGBoost": (0.7691, 0.6819, 0.0264),
    "Random Forest": (0.7731, 0.6770, 0.0088),
    "LightGBM": (0.7600, 0.6770, 0.0264),
    "Decision Tree": (0.7460, 0.6737, 0.0396),
    "Logistic Regression": (0.7721, 0.6728, 0.0000),
    "KNN": (0.7369, 0.6700, 0.0441),
}

x = np.arange(len(prompt_results))
width = 0.35
names = list(prompt_results.keys())

prompt_f1 = [prompt_results[n][1] for n in names]
skill_f1 = [cv_results[n]["test_f1_weighted"][0] if n in cv_results else np.nan for n in names]
prompt_ur = [prompt_results[n][2] for n in names]
skill_ur = [cv_results[n]["test_recall_unemp"][0] if n in cv_results else np.nan for n in names]

ax2 = ax.twinx()
b1 = ax.bar(x - width/2, prompt_f1, width, label="Prompt版 F1", color=PALETTE[0], edgecolor="white", alpha=0.7)
b2 = ax.bar(x + width/2, skill_f1, width, label="Skill版 F1 (CV)", color=PALETTE[1], edgecolor="white", alpha=0.7)
ax.scatter(x - width/2, [v * 15 for v in prompt_ur], marker="v", s=80, color="red", zorder=5, label="Prompt 失业召回×15")
ax.scatter(x + width/2, [v * 15 for v in skill_ur], marker="^", s=80, color="green", zorder=5, label="Skill 失业召回×15")

ax.set_xticks(x); ax.set_xticklabels(names, fontsize=8, rotation=30, ha="right")
ax.set_ylabel("F1 分数", fontsize=12); ax.set_title("Prompt版 vs Skill增强版 对比", fontweight="bold")
ax.legend(loc="upper left", fontsize=8)
ax2.legend(loc="upper right", fontsize=8)
ax.set_ylim(0, 1); ax.grid(axis="y", alpha=0.3)

plt.tight_layout(); save_chart("04_prompt_vs_skill.png")
